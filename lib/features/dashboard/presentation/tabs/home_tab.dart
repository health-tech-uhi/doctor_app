...
class HomeTab extends ConsumerWidget {
          ...
          child: CustomScrollView(
            ...
            slivers: [
              if (authState.requiresProfileCompletion)
                ...
                SliverToBoxAdapter(
                  child: ref.watch(doctorProfileProvider).when(
                    loading: () => const SizedBox(height: 120),
                    error: (error, _) => _buildHeader(context, 'Doctor'),
                    data: (p) => _buildHeader(context, p.fullName.split(' ').first),
                  ),
                ),
              SliverPadding(
              ...
              ..._buildTodayAppointmentSlivers(context, ref.watch(appointmentsListProvider)),
  ...
  Widget _buildStatsGrid(
    ...
    final today = appointmentsAsync.maybeWhen(data: (all) => appointmentsOnDay(all, now).length, orElse: () => 0);
    final upcoming = appointmentsAsync.maybeWhen(data: (all) => appointmentsUpcoming(all, now).length, orElse: () => 0);
    final waiting = appointmentsAsync.maybeWhen(data: (all) => all.where((a) => a.normalizedStatus == 'requested').length, orElse: () => 0);
    final patients = relationshipsAsync.maybeWhen(data: (all) => all.length, orElse: () => 0);
    ...
    return GridView.count(
      ...
      children: [
        _statCard('Today', today.toString(), Icons.calendar_today_rounded, DoctorTheme.accentCyan),
        _statCard('Patients', patients.toString(), Icons.group_outlined, DoctorTheme.accentLavender),
        _statCard('Upcoming', upcoming.toString(), Icons.bolt_rounded, DoctorTheme.accentAmber),
        _statCard('Waiting', waiting.toString(), Icons.hourglass_empty_rounded, const Color(0xFFFF6B6B)),
      ],
    );
  }
  ...
  List<Widget> _buildTodayAppointmentSlivers(BuildContext context, AsyncValue<List<Appointment>> appointmentsAsync) {
    return appointmentsAsync.when(
      loading: () => [const SliverToBoxAdapter(child: Center(child: Padding(
        padding: EdgeInsets.all(40),
        child: CircularProgressIndicator(strokeWidth: 2),
      )))],
      error: (e, _) => [const SliverToBoxAdapter(child: Center(child: Text('Connect to load schedule')))],
      data: (all) {
            ...
            SliverToBoxAdapter(
              child: Center(
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      Icon(Icons.event_available_outlined, size: 48, color: DoctorTheme.textTertiary.withValues(alpha: 0.5)),
                      const SizedBox(height: 12),
                      const Text('Rest day — No appointments', style: TextStyle(color: DoctorTheme.textTertiary)),
                    ],
                  ),
                ),
              ),
  ...
  Widget _buildAppointmentCard(Appointment appt) {
    ...
    final color = status == 'accepted' ? DoctorTheme.accentCyan : status == 'requested' ? DoctorTheme.accentAmber : DoctorTheme.textTertiary;
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      appt.displayReason, 
                      style: const TextStyle(color: DoctorTheme.textSecondary, fontSize: 13)
                    ),
                  ],
                ),
              ),
              Column(
                ...
                children: [
                  Text(
                    ...
                    style: const TextStyle(fontWeight: FontWeight.bold, color: DoctorTheme.accentCyan)
                  ),
