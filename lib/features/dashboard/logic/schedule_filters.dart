...
bool sameCalendarDay(DateTime a, DateTime b) => _startOfDay(a) == _startOfDay(b);
...
List<Appointment> appointmentsOnDay(List<Appointment> all, DateTime day) {
  final out = all.where((a) => sameCalendarDay(a.requestedDatetime, day)).toList();
...
List<Appointment> appointmentsUpcoming(List<Appointment> all, DateTime now) {
  ...
  final out =
      all.where((a) => !a.requestedDatetime.isBefore(endOfToday)).toList();
  out.sort((a, b) => a.requestedDatetime.compareTo(b.requestedDatetime));
  return out;
}
...
List<Appointment> appointmentsPast(List<Appointment> all, DateTime now) {
  ...
  final out = all.where((a) => a.requestedDatetime.isBefore(startToday)).toList();
